...
**Status:** In progress — v1.6.9: Channels grid single-click opens editor + readable active-row highlight + flat column headers; v1.6.8: Android backup Review diff button + skip redundant import dialog; health duplicate DMR ID check; v1.6.7: TG/Rx Group List editor layout + filter (Zone editor parity); v1.6.6: Tier 1.2 fork app icon (`DMR_Fork.ico`) + F8 backup shortcut; v1.6.5: Zone editor layout; v1.6.4: Tier 1.7 health drill-down (dup names, zone table, orphan zones) + Zone editor Available filter; v1.6.3: Tier 2.1 Contacts grid filter/sort parity with Channels; v1.6.2: auto-open workspace + Contacts G/P badges; v1.6.1: Health toolbar + F7; v1.6.0: prominent WebView2 health/backup; v1.5.9: Tier 3.1 WebView2 HTML report in Android backup manager; v1.5.8: Tier 2.1 D/A + G/P/A grid badges; dead code cleanup (`ChannelsCsvImporter`, `VoteScanForm`); v1.5.7: Channels toolbar 3-row layout; v1.5.6: Tier 1.4 splash + Tier 2.1 grid sort/filter; v1.5.5: Tier 1.2 Segoe UI typography; v1.5.4: Tier 1.5 advanced default on; v1.5.3: RadioID link + hide Analog on digital+collapsed; v1.5.2: Tier 1.5 collapsible advanced codeplug fields; v1.5.1: Tier 1.1 status bar hint + Tier 1.4 About links; v1.5.0: Tier 1.1 menu declutter + help/shortcuts; v1.4.9: Tier 1.7 codeplug health panel; v1.4.8: Contacts double-click lookup fix; v1.4.7: Contacts toolbar layout; v1.4.5–1.4.6: Tier 3.4 DMR ID lookup; v1.4.4: channel grid click-to-sync + v1.4.3 double-click fix; v1.4.2: Tier 2.3 docking UX (default layout, reset, persist); v1.4.0: Tier 2.8 grid bulk edit + Tier 2.9 column visibility + Tier 1.2 dark dock tabs; v1.3.6: Tier 2.4 import/export summary dialog + busy overlay; v1.3.5: push folder any safe name; v1.3.4: Tier 2.10b ADB push export to phone; v1.3.3: Tier 2.6 contact integrity checker + DPI manifest; v1.3.2: Tier 2.10 ADB pull for phone backups; v1.3.1: MTP copy-to-PC folder picker; v1.3.0: Tier 2.5 pre-import channel diff (Apply/Cancel); v1.2.7: File menu layout fix; v1.2.6: label fix all editors, channel filter, grid stripes, import preview counts; v1.2.4–5 validation/Ctrl+Z; v1.2.1–3 Tier 1 shell
...
| **Typography** | Default font **Segoe UI** 9–10pt (or Segoe UI Variable on Windows 11). **Shipped:** v1.5.5 — `Theme.UiFont` on shell + primary editors. | `Theme.cs`, editor forms |
...
|----------|-------------|
| Optional splash | Short splash on startup with fork name + warning. **Shipped:** v1.5.6 — `ForkSplashForm` once per version. |
...
**Shipped:** v1.5.4 — advanced checkbox defaults on; v1.5.3 — RadioID link + hide Analog on digital+collapsed; v1.5.2 — collapsible advanced; v1.2.x+ — Android GroupBox + tooltips.
...
| **Collapsible advanced** | **Shipped v1.5.2** — “Show advanced codeplug fields” hides privacy/signaling/OEM controls; compact Digital/Analog groups. | `ChannelForm.cs` |
...
| Improvement | Description | Primary files |
|-------------|-------------|---------------|
| **DataGridView polish** | Double-buffering, alternating row colors, clear Analog/Digital column or icon. **Shipped:** v1.5.8 — `D`/`A` mode badges + color-coded `G`/`P`/`A` type badges. | `ChannelsForm.cs` |
| **Sorting / filter** | Sort by channel number, name, type; quick filter box. **Shipped:** v1.5.6 — header-click sort; multi-column filter (v1.2.6 name-only extended). | `ChannelsForm.cs` |
...
**Shipped:** v1.4.2 — `ForkDockLayout.cs`, `DockPanel.config.default`; first-run Channels + channel 0 tile; View → Restore PriInterPhone default layout; save on exit.
...
**Shipped:** v1.3.6 — `AndroidBatchResult.cs`, `AndroidBatchResultForm.cs`, `AndroidBusyForm.cs`; wired from `MainForm.ImportAndroidBackupFolder` / `ExportAndroidBackupFolder` (scrollable log, marquee busy overlay, post-import integrity warnings).
...
**Shipped (spike):** v1.5.9 — `ForkWebViewPanel` + `AndroidBackupReportHtml` in `AndroidBackupForm` (Log + Report tabs). Graceful fallback when WebView2 runtime missing.
